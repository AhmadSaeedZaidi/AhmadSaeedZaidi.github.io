...
- The directional derivative (notation page) gives the mathematical foundation: $D_v f(\theta) = \nabla f(\theta) \cdot v$ is the slope along direction $v$. The negative gradient gives the steepest descent direction because it maximizes $\lvert \nabla f \cdot v \rvert$ among all unit vectors $v$.
